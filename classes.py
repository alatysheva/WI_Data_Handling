...
class XmlGeneration:
    ...
    def lastoxml(self, lf, filename, uidWell, uidWellbore, BU, asset, purpose1, servicecompany, wellname1, idwi, runid,
        ...
        for well in lf.well:
            # if well.descr.lower().find('well') != -1 and well.descr.find('ID') == -1:
            # wellname = well.value
            if well.descr.lower().find('wellbore') != -1 and well.descr.find('ID') == -1:
                wellbore = well.value
            # elif well.descr.lower().find('service') != -1:
            # SC = well.value
            elif well.descr.lower().find('run') != -1:
                runNumber = well.value
            elif well.descr.lower().find('date') != -1:
                creationDate = well.value
            elif well.descr.lower().find('null') != -1:
        ...
        for curve in lf.curves:
            ...
            if indexType == 'date time':
                ...
                child2 = SubElement(top_2, 'minIndex')
                child2.text = str(startIndex)
                child3 = SubElement(top_2, 'maxIndex')
    ...
    def csvtoxml(self, df, df2, x, c, filename, uidWell, uidWellbore, BU, asset, purpose1, servicecompany, wellname1,
        ...
        j = 1
        for col in df2.columns:
            top_2 = SubElement(top_1, 'logCurveInfo', uid=col)
            child1 = SubElement(top_2, 'mnemonic')
            child1.text = col
            child1a = SubElement(top_2, 'unit')
            child1a.text = str(units[j - 1])
        ...
        for i in range(c, len(df)):
            top_3_3 = SubElement(top_3, 'data')
            if x is None or x == '':
                top_3_3.text = ','.join(str(v) for v in df.iloc[c - 1 + p].to_list())
            else:
